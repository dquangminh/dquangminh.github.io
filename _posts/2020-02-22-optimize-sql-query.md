...
Làm việc với cơ sở dữ liệu quan hệ là làm việc với các bảng. Dữ liệu cần truy vấn có thể nằm ở các bảng riêng biệt và thông qua mệnh đề FROM để JOIN các bảng tạo thành bảng làm việc (working table). Nhưng chúng ta không sử dụng toàn bộ dữ liệu của bảng này do đó có thêm mệnh đề WHERE để loại bỏ bớt các bản ghi không thỏa mãn điều kiện đặt ra và nhiều khi dữ liệu từ bảng làm việc nhỏ hơn rất nhiều so với dữ liệu thỏa mãn điều kiện của mệnh đề WHERE.

Chẳng hạn, một cơ sở dữ liệu lưu trữ thông tin của tất cả người dùng và cả những shops mà nó quản lý. Để tìm kiếm tất cả users và shops đặt tại cùng thành phố H. Ta có thể thực hiện như sau
...
Ở câu truy vấn này không có giá trí cụ thể nào được tìm kiếm do vậy nó sẽ sử dụng lợi ích thứ 2 đó là trả về các hàng theo thứ tự index (do cả 2 cột của 2 bảng đều được đánh index). Do tận dụng được lợi ích thứ 2 đó là các hàng theo thứ tự index do đó thay vì thực hiện CROSS JOIN 2 bảng tốn kém chi phí hiệu năng hơn nó chỉ cần thực hiện MERGE JOIN (merge chỉ thực hiện được các tập hợp có thứ tự). 

So sánh phép CROSS JOIN với N*M = 10 triệu và MERGE JOIN với N+M ~ 1 triệu có thể thấy lợi ích của index trong TH này.
...
Khi xem xét về hiệu năng truy vấn rõ ràng đánh index cần phải đưa lên vị trí đầu tiên bởi hiệu quả ngay tức khắc của nó.

Tuy nhiên tôi thường tập trung xem xét việc đánh index một cách kỹ lương ở bước tạo lược đồ bởi hiệu năng có được này là một sự đánh đổi, hiệu năng truy vấn sẽ phải đánh đổi cho hiệu năng update/insert/delete. Do đó cần phải xem xét cân bằng điều này nếu hiệu năng update/insert/delete không phải là vấn đề lớn ở đây thì index chắc chắn là tối ưu hàng đầu. tuy nhiên nếu đó là một vấn đề thực sự thì rõ ràng việc thêm index vào sẽ là một ý tưởng tồi, thay vào đó cần xem xét việc tối ưu sử dụng các index đã có sẵn có khả thi không cùng nhiều thủ thuật khác để đạt được được mục tiêu.

Khi mà lược đồ vẫn còn được đánh giá là tốt thì việc sửa đổi hay cập nhật index sẽ cần phải được xem xét kỹ lưỡng hơn. Hơn thế nữa khi có quá nhiều index xuất hiện nó có thể đánh lừa optimizer sử dụng nó trong khi không nên sử dụng bởi trình tối ưu hóa có xu hướng dựa vào index rất nhiều.
